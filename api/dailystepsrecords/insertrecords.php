...
if (mysqli_num_rows($query) > 0) {
    ...
    $sql = "Insert into daily_steps_records(user_id,calories_burnt,distancecovered,time_taken,avg_speed,avg_pace,date,steps) Values ('{$user_id}','{$calories_burnt}','{$distancecovered}','{$time_taken}','{$avg_speed}','{$avg_pace}','{$created_date}','{$steps}')";
    $query = mysqli_query($conn, $sql);
    $response = array('distancecovered' => $distancecovered, 'calories_burnt' => $calories_burnt, 'time_taken' => $time_taken, 'created_date' => $created_date, 'steps' => $steps, 'Averagepace' => $avg_pace, 'message' => 'Inserted createRecord', 'error' => false);
}
